...
async def embed(ctx):
    embed=discord.Embed(title="**help command list of iso6.9**", description="current version: 22122021a\ncurrent prefix: `]`", color=discord.Color.blue())
    ...
    embed.add_field(name='misc:', value="snipe (channel), edit_snipe (global), 8ball, fstab, roll, say, null, sus, notify, stroke, randnum, kill, amogus", inline=False)
...
async def embed77(ctx):
    ...
    embed.add_field(name='misc:', value="snipe (channel), edit_snipe (global), 8ball, fstab, roll, say, null, sus, notify, stroke, randgen, kill, amogus", inline=False)
    ...
    await ctx.reply(embed=embed)
    print(f'[log] {ctx.author.name} requested ]ahelp.')
...
async def kill(ctx, user: discord.User, weapon: str):
    await ctx.send(f'{ctx.author.name} killed {user} with {weapon}, ouch.')
    print(f'[log] {ctx.author.name} requested ]kill.')

@bot.command()
async def amogus(ctx):
    await ctx.reply("https://c.tenor.com/1iSARWJr-TEAAAAC/among-us-twerk.gif")
    print(f'[log] {ctx.author.name} requested ]amogus.')
