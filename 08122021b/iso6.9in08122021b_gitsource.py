...
bot = commands.Bot(command_prefix=']', intents=intents, activity=discord.Activity(type=discord.ActivityType.watching, name="everyone | ]chelp"), status=discord.Status.idle)
status = 'idle'
...
async def embed(ctx):
    embed=discord.Embed(title="**help command for iso6.9**", description="prefix: `]`", 
    color=discord.Color.blue())
    embed.add_field(name='moderation:', value="kick, ban, unban, purge, mute, unmute", inline=False)
    embed.add_field(name='bot info:', value="testcmd, ping, serverlist, morebots, serverinfo, userinfo", inline=False)
    embed.add_field(name='misc:', value="snipe (channel), edit_snipe (global), 8ball, fstab, roll, say, null", inline=False)
    embed.add_field(name='disabled:', value="<no commands>", inline=False)
    embed.set_footer(text="> type ]chelp to get this list.\n(Information requested by: {})".format(ctx.author.display_name))
