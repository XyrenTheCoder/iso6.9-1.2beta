...
# var
token = "OTE1NDg4MDg3NTU0MDAyOTU2.YacUuQ.kiqb-0TFKv2bLAE25DuiWDntiSM"
cid = "915488087554002956"
csecret = "et4TSc88R3oR2uaOwUQO3hEX6T2Ke_L9"
owner = [
        "αrchιshα#5518",
        "notsniped#4573",
...
async def embed(ctx):
    embed=discord.Embed(title="**help command list of iso6.9**", description="current version: 03012022a\ncurrent prefix: `]`", color=discord.Color.blue())
    ...
    embed.add_field(name='misc:', value="snipe (channel), edit_snipe (global), 8ball, fstab, roll, say, null, sus, notify, stroke, randnum, kill, amogus, guess, duel, cup, lick", inline=False)
...
async def embed77(ctx):
    ...
    embed.add_field(name='misc:', value="snipe (channel), edit_snipe (global), 8ball, fstab, roll, say, null, sus, notify, stroke, randgen, kill, amogus, guess, duel, cup, lick", inline=False)
...
async def purge(ctx, amount: int):
    if int(amount) == 0:
        return await ctx.reply("You can\'t purge 0 messages **dood**.")
    elif int(amount) < 0:
        return await ctx.reply("You can\'t spawn messages, lol.")
    elif int(amount) > 500:
        return await ctx.reply("You can\'t purge more than 500 messages :face_with_raised_eyebrow:")
    await ctx.message.delete()
